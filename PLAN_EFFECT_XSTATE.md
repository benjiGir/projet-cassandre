...
> **✅ Livré (2026-09-03), fait directement (pas délégué — transformation mécanique d'une fonction déjà entièrement lue, pas de nouvelle conception IA/architecture).** `updateGameplay`/`stepPhysics` (`src/main.ts`) réorganisés en un seul `Effect.gen`/`Effect.sync` chacun, exécuté via `runGameplaySync` — mêmes statements, même ordre exact, aucune ligne de logique changée (vérifié par relecture du diff : chaque bloc de code est identique, seuls des `yield* Effect.sync(() => ...)` et l'indentation ont été ajoutés). `stepPhysics` n'est qu'un `Effect.sync` unique (trivial). Test d'intégration ajouté (`test/game/integration/gameplayStep.test.ts`) : construit un vrai `WeaponSystem`/`SuitManager`/`Suit` contre un vrai monde Rapier, un coup de pied-de-biche résout un dégât ET fait transitionner l'état (`idle` → `stagger`) à travers la vraie chaîne d'appels (`WeaponSystem.update → SuitManager.update → Suit.applyDamage`), chacune passant déjà par `runGameplaySync(RaycastService.use(...))` (M3) — si l'un de ces appels suspendait, le test échouerait de lui-même. Piège Rapier déjà connu (M3/M4) retrouvé et corrigé du premier coup (`physics.step(0)` après spawn, avant tout raycast). `pnpm build`/`pnpm test` verts (98/98).
>
> **Réduction de périmètre décidée avant implémentation (2026-09-03), à partir d'une lecture directe de `main.ts` actuel.** Trois écarts délibérés par rapport à la conception ci-dessous, chacun documenté plutôt que silencieux :
>
> 1. **Pas de `PlayerService`/`WeaponService`/`EntityManagerService`.** `PlayerController`/`WeaponSystem`/`SuitManager`/`DirectorManager` restent des classes plates. Aucun besoin concret identifié aujourd'hui qui justifierait de les envelopper en `Context.Service` Effect — ce serait de l'abstraction avant que la douleur soit réelle (invariant #9). Si un futur jalon a un vrai besoin de substituer l'un de ces systèmes entiers via une `Layer` de test (pas juste ses raycasts/pathfinding, déjà couverts par M3/M4), ce sera le bon moment.
> 2. **Le flag `isDead`/`isLevelComplete` n'est PAS remplacé.** Le texte ci-dessous le dit remplacé par "une lecture d'état de la machine de flux d'écran" — cette machine n'existe pas encore, elle est prévue pour M8 (pas encore livré à ce stade). Le remplacement reste explicitement le travail de M8, quand la machine existera réellement ; y toucher ici créerait une dépendance dans le mauvais sens.
> 3. **Pas d'extraction de `updateGameplay` hors de `main.ts`.** `updateGameplay` ferme sur ~25 variables locales mutables de `main()` (`hasBadge`, `playerHp`, `badgeMesh`, `openingDoor`, `exitDoorTracking`, `foundSecrets`, `currentNavGraph`, `gltfLevelSession`, `unlockedDoors`...). L'extraire proprement en module testable indépendamment aurait exigé de restructurer tout cet état mutable de `main()` en un objet de session explicite — un chantier bien plus large et risqué que ce que ce jalon vise, sur le fichier le plus central et sensible du jeu. `updateGameplay` reste donc EN PLACE dans `main.ts`, son corps réorganisé en un seul `Effect.gen` (phases nommées : capture d'input, `player.update`, `interaction.update`, `weapons.update`, `suitManager`/`directorManager.update`, résolution badge/porte/sortie/secrets), exécuté via `runGameplaySync` — même séquence exacte qu'aujourd'hui, aucune réorganisation. Le critère d'acceptation "test d'intégration hors navigateur" est satisfait autrement : un test qui construit de VRAIES instances `WeaponSystem`/`SuitManager`/`Suit` et vérifie qu'un tir résout bien un dégât à travers la chaîne réelle (`weapons.fire* → hitEvents → suitManager.update → Suit.applyDamage`), chacune de ces étapes passant déjà par `runGameplaySync`/`RaycastService` (M3) — la même preuve que demande le plan, sans le risque de l'extraction.
>
> `stepPhysics` (juste `physics.step(dt)` + synchronisation de la balle du gym) reçoit le même traitement (`Effect.sync` unique, exécuté via `runGameplaySync`) — trivial, mentionné nulle part explicitement dans la conception d'origine mais fait partie du pas fixe au sens de l'invariant #1, donc logiquement du périmètre de ce jalon plutôt que de M7 (qui reste `interpolateVisuals`/`updateFx`/`render`, le domaine du TAUX D'AFFICHAGE, pas du pas fixe).
...
> **✅ Livré (2026-09-03).** `src/ui/gameFlowMachine.ts` : machine pure (7 états, 9 événements), aucune dépendance au jeu — `main.ts` pousse son état dans `useGameStore` via `subscribe()`. `main.ts` réécrit autour d'une `interface GameSession` regroupant tout l'état PAR PARTIE (physics/player/weapons/managers/sprites/session de niveau/badge/portes/secrets/PV) ; `bootGameSession(choice)`/`teardownGameSession(session)` construisent/détruisent cet ensemble, réutilisées au boot initial ET à chaque `replay()`/`returnToMenu()`. `scene`/`camera`/`renderer`/`clock`/audio restent persistants. `physics.world.free()` en dernier au teardown (API Rapier confirmée libérer bodies/colliders/KCC en un coup, donc aucune méthode `dispose()` ajoutée à `SuitManager`/`DirectorManager`/`WeaponSystem`/`PlayerController`). `isDead`/`isLevelComplete` retirés de `state.ts`, le garde de `updateGameplay` (M6) lit directement l'état de l'acteur — exactement le travail que M6 avait reporté ici. `ui/screenNav.ts` supprimé (plus aucun appelant). Bug préexistant trouvé et corrigé en passant : `directorManager.snapshotPrevious()` n'était jamais appelé.
>
> Détail non trivial repéré et bien géré par l'agent : `spawnSuitAt`/`spawnDirectorAt`/`loadGltfLevel` prennent `session` en paramètre EXPLICITE plutôt que de fermer sur la variable mutable `currentSession` — nécessaire car ces fonctions tournent aussi PENDANT la construction d'une nouvelle session, avant qu'elle ne devienne "la" session courante (sans ça, un spawn pendant un reset aurait pu atterrir dans l'ancienne partie). `isPhysicsSessionLive()` ajouté pour garder `stepPhysics` pendant la fenêtre transitoire où l'ancien monde Rapier est déjà libéré mais le nouveau pas encore construit (retour au menu).
>
> **Vérifié par cette session** (relecture complète du diff, pas seulement le rapport de l'agent) : `bootGameSession`/`teardownGameSession`/`replay`/`returnToMenu`/`gameFlowMachine.ts` lus intégralement, cohérents avec la conception. `pnpm build`/`pnpm test` verts (116/116, dont 15 tests de la table de transition complète + scénarios composés, et un test d'intégration réel Rapier de boot→teardown→reboot). **Vérification en jeu réelle partielle** : boot → menu → "Jouer" → niveau complet chargé et rendu correctement, zéro erreur console inattendue, aucun rechargement de page (marqueur JS injecté en console survit au chargement). **Mourir → rejouer et terminer un niveau → retour menu n'ont PAS pu être vérifiés en direct** : le panneau Browser de cette session tourne avec `document.visibilityState: hidden` (la même limitation documentée pour chaque phase précédente de ce projet — la boucle à pas fixe se fige entièrement tant que l'onglet n'est pas réellement visible, aucune interaction ne le débloque), empêchant toute progression de combat en temps réel nécessaire pour atteindre la mort organiquement. À valider en jouant réellement.
>
> **Précisions faites avant délégation (2026-09-03), à partir d'une lecture directe de `main.ts`/`screenNav.ts`/`App.tsx`/`DeathScreen.tsx`/`LevelCompleteScreen.tsx`.** C'est la plus grosse construction de fonctionnalité nouvelle après M4 — aucun chemin de reset n'existe aujourd'hui (`CLAUDE.md` documente que ça a été jugé disproportionné en Phase 6 ; ce chantier est le bon moment). Détruit et reconstruit à un reset : `PhysicsWorld` (donc `player`/`weapons`/`suitManager`/`directorManager`, tous construits à partir de `physics`), la session de niveau, tout l'état de suivi par partie (badge/porte/sortie/secrets/sprites), l'état debug. Reste persistant : `scene`/`camera`/`renderer`, le root React, `input`/audio/musique déjà initialisés, `GameClock`, l'acteur de flux lui-même. Nettoyage Rapier obligatoire (`physics.world.free()`, déjà utilisé comme précédent dans `simulateRecording`) — sans ça, fuite de mémoire WASM à chaque reset. Comme `PhysicsWorld` est entièrement recréé, `SuitManager`/`DirectorManager`/`WeaponSystem`/`PlayerController` n'ont pas besoin de méthodes `dispose()` propres — ils sont juste recréés. Architecture recommandée : extraire la construction PAR PARTIE de `main()` en une paire `bootGameSession`/`teardownGameSession` réutilisable au boot initial ET à chaque reset — une vraie restructuration de `main.ts`, contrairement à M6 où l'extraction avait été jugée disproportionnée : ici c'est le mécanisme même qui rend le reset possible. `isDead`/`isLevelComplete` (deux flags dans `state.ts`) sont retirés au profit d'une lecture de l'état de cette machine — le garde en tête d'`updateGameplay` (M6) est mis à jour pour ça, exactement le travail que M6 avait explicitement reporté ici.
>
> Délégué à `shell`. Vérification en jeu réelle **faite par cette session, pas par l'agent** (le Browser pane n'est disponible que dans cette session, découvert en M7) — parcours complet boot → menu → jouer → mourir → rejouer → terminer un niveau → retour menu, à faire après le rapport de l'agent.
