...
import { moveConfig } from "../game/player/moveConfig";
import { weaponConfig } from "../game/player/weaponConfig";

/**
 * Réglages graphiques et visuels — écran « Options › Affichage »
 * (`RebindScreen.tsx`). Module NON visuel : persistance `localStorage` +
 * application aux systèmes concernés. N'IMPLÉMENTE aucun des quatre
 * réglages lui-même — il APPELLE les fonctions déjà exportées de
 * `render/renderer.ts` et mute les objets déjà mutables de `moveConfig.ts`/
 * `weaponConfig.ts` (même pattern que `TuningPanel.tsx`, qui fait exactement
 * ça pour ses propres sliders).
 *
