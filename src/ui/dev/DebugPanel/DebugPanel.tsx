...
export function DebugPanel() {
  ...
  return (
    <div className={styles.panel}>
      ...
      <div>{`Jeu p95 (120 images): ${debug.gameplayP95Ms.toFixed(2)}ms`}</div>
      <div>{`A*: ${debug.astarQueries} requêtes, ${debug.astarMisses} échecs, ${debug.astarExpandedNodes} nœuds`}</div>
      <div>{`A* dernier/max: ${debug.astarLastMs.toFixed(2)}/${debug.astarMaxMs.toFixed(2)}ms`}</div>
